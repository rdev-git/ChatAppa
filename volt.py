...
KEYWORDS = [                # ключевые слова для пакета
    "offline", "cars", "pacman", "weteers",
    "apple", "dating", "mark", "fly",
    "power", "private", "wool", "pretty",
    "json", "app", "volt", "unknown"]
...
def buildBundles(path):
    ...
    for i in range(BD_COUNT):
        ...
        log("Сборка {}.aab, пакет: {}".format(end, ORIGINAL_PACKAGE + "." + end))
        changePackage(path, end)
        buildBundle(path, str(i + 1))
...
def buildBundle(path, out):
    # сборка апк
    os.system("gradlew bundleRelease".format(PROJ_DIR))
    # копируем апк
    os.system("copy app\\build\\outputs\\bundle\\release\\app-release.aab {}.aab".format(out))
    # добавляем в архив
    #os.system("7z a _bundles.zip {}.aab".format(out))
    return
